...
for epoch in range(500):
    ...
    # L3: 邻居一致性损失
    loss_l3 = neighbor_loss(z, knn_neighbors)

    # 总损失
    loss = 2.0 * loss_l2 + loss_l3
    ...
    if epoch % 50 == 0 or epoch == 499:
        ...
        with torch.no_grad():
            ...
            if sil > best_sil:
                best_sil = sil
                best_epoch = epoch
            print(f'Epoch {epoch:3d}: Loss={loss.item():.4f} (L2={loss_l2.item():.4f}, L3={loss_l3.item():.4f}), Sil={sil:.4f}, Best={best_sil:.4f}')

print('\n' + '=' * 60)
print('结果对比')
print('=' * 60)
print(f'L2+L3 最佳 Silhouette: {best_sil:.4f} (Epoch {best_epoch})')
print(f'L2 alone Silhouette: 0.3030')
print(f'V2.3 Full (L1+L2+L3): 0.40')
print(f'理论上限: {sil_upper:.4f}')

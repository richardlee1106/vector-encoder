# -*- coding: utf-8 -*-
"""
P2: Region Sep 优化实验 - Center Loss

目标：
1. 提升 Region Sep 从 0.65 到 > 2.0
2. 保持 Pearson > 0.96
3. 保持 DirAcc > 75%
...
class IntegratedMultiTaskLoss(nn.Module):
    ...
    def __init__(
        ...
        region_weight: float = 2.0,
        center_weight: float = 0.2,      # P2: Center Loss权重
        ...
        # P2-Phase2B: Center Loss 作用于 embedding 层
        if center_weight > 0:
            from spatial_encoder.v26_GLM.losses_v26_pro import CenterLoss
    ...
    def forward(
        ...
        # ============ 5. P2-Phase2B: Center Loss（作用于embedding层） ============
...
def load_data_integrated(config: V26ProConfig, sample_ratio: float = 0.1, use_neighbor_feats: bool = True, use_pseudo_labels: bool = False) -> Dict:
    ...
    try:
        ...
        print("Using cached pseudo-labels with mock features...")

        # Fallback: 使用伪标签缓存 + 模拟特征
        from pathlib import Path
        ...
        if pseudo_path.exists():
            ...
            N = len(pseudo_labels)
            print(f"  Loaded pseudo-labels: {N} cells, {(pseudo_labels < 6).sum()} valid ({(pseudo_labels < 6).sum()/N*100:.1f}%)")
        else:
            # 完全模拟数据
            N = 67138  # 与 P1D 数据集大小一致
            pseudo_labels = np.random.randint(0, 7, N)
            confidence = np.random.rand(N)
            print(f"  Using mock data: {N} cells")
        ...
        if sample_ratio < 1.0:
            ...
            N = sample_size
            print(f"  Sampled to {N} cells")

        # 生成模拟坐标（武汉范围）
        np.random.seed(42)
        coords = np.zeros((N, 2), dtype=np.float32)
        coords[:, 0] = np.random.uniform(113.6, 115.0, N)  # 经度
        coords[:, 1] = np.random.uniform(29.9, 31.3, N)    # 纬度

        # 计算邻居索引
        print("  Computing neighbor indices...")
        nbrs = NearestNeighbors(n_neighbors=10, algorithm='ball_tree').fit(coords)
        _, neighbor_indices = nbrs.kneighbors(coords)
        ...
        for i in range(N):
            if len(neighbor_indices[i]) > 0:
                neighbor = neighbor_indices[i][0]
                dx = coords[neighbor, 0] - coords[i, 0]
                dy = coords[neighbor, 1] - coords[i, 1]
                angle = np.arctan2(dy, dx)
                neighbor_labels[i] = int((angle + np.pi) / (np.pi / 4)) % 8
                neighbor_valid[i] = True
        ...
        return {
            ...
            "global_dir_labels": global_labels,
            "global_dir_valid": global_valid,
            "region_labels": pseudo_labels,  # 使用伪标签
            "original_region_labels": pseudo_labels,  # 模拟情况下相同
        }
...
class TrainerP1C:
    ...
    def train(self, train_loader, val_loader, epochs: int = 30):
        ...
        # 保存模型
        save_dir = Path(__file__).parent / "saved_models" / "v26_pro"
        save_dir.mkdir(parents=True, exist_ok=True)
        save_path = save_dir / "l3_trained_model.pt"
...
def run_p1c_experiment(
    ...
    epochs: int = 80,                # P2-Phase2C: 提高到80
    batch_size: int = 16384,
    use_neighbor_feats: bool = False,  # P1E: 禁用邻域特征（已证明无效）
    use_pseudo_labels: bool = True,   # P1F: 启用伪标签（44% 覆盖率）
    distance_weight: float = 0.5,     # P2-Phase2C: 保持不变
    region_weight: float = 1.5,       # P2-Phase2C: 回退到最佳配置
    center_weight: float = 0.2,       # P2-Phase2C: 回退到最佳配置
):
    print(f"\n{'#'*60}")
    print(f"# P2-Phase2C: Full Training with Best Config")
    print(f"# Config: distance_weight={distance_weight}, region_weight={region_weight}, center_weight={center_weight}")
    print(f"# Target: Pearson > 0.97, DirAcc > 75%, ClfF1 > 55%, Region Sep > 1.1")
    if use_pseudo_labels:
    ...
    config.loss.center_weight = center_weight  # P2: 设置Center Loss权重
    ...
    # 损失函数 - P2-Phase2 使用传入的权重参数
    reconstruction_weight = 0.3
    direction_weight = 1.5

    criterion = IntegratedMultiTaskLoss(
        k_nearest=config.loss.k_nearest_neighbors,
        distance_weight=distance_weight,      # P2-Phase2: 使用参数
        reconstruction_weight=reconstruction_weight,
        direction_weight=direction_weight,
        region_weight=region_weight,          # P2-Phase2: 使用参数
        center_weight=center_weight,          # P2: Center Loss
        neighbor_dir_weight=0.6,
        global_dir_weight=0.4,
        use_focal=True,
        focal_gamma=2.0,
    )
    print(f"P2-Phase2 Loss weights: distance={distance_weight}, reconstruction={reconstruction_weight}, "
          f"direction={direction_weight}, region={region_weight}, center={center_weight}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="P2-Phase2C: Full Training with Best Config")
    parser.add_argument("--sample", type=float, default=1.0, help="Sample ratio (default: 1.0 for full training)")
    ...
    parser.add_argument("--distance-weight", type=float, default=0.5, help="Distance loss weight")
    parser.add_argument("--region-weight", type=float, default=1.5, help="Region classification weight")
    parser.add_argument("--center-weight", type=float, default=0.2, help="Center Loss weight")

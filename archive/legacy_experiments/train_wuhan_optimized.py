...
def load_wuhan_sample(sample_rate: float = 0.1):
    ...
    print(f"加载了 {len(poi_coords):,} 个POI")
...
def train_optimized(data):
    """使用优化配置训练"""
    print("\n" + "=" * 60)
    print("V2.3 武汉全量 - 优化版训练")
    print("=" * 60)
    print("配置: epochs=800, lr=1e-3, w1=1.0, w2=2.0, w3=1.0, n_clusters=30")
    ...
    for epoch in range(800):
        ...
        if epoch % 50 == 0 or epoch == 799:
            ...
            print(f"  Epoch {epoch:3d}: Loss={loss.item():.4f}, Sil={sil:.4f}, Best={best_sil:.4f} ({rate:.1f}%)")

            history.append({'epoch': epoch, 'sil': sil, 'best_sil': best_sil})
    ...
    print("\n对比分析:")
    print("-" * 50)
    print(f"{'数据集':<20} {'Silhouette':<12} {'达成率'}")
    print("-" * 50)
    print(f"{'guanggu_core':<20} {'0.4153':<12} {'92.4%'}")
    print(f"{'wuda_area':<20} {'0.4075':<12} {'89.6%'}")
    print(f"{'zhongjia_cun':<20} {'0.3756':<12} {'90.3%'}")
    print(f"{'实验区平均':<20} {'0.3994':<12} {'90.7%'}")
    print("-" * 50)
    print(f"{'武汉10%采样':<20} {best_sil:.4f}       {best_sil / sil_upper * 100:.1f}%")

    if best_sil / sil_upper >= 0.85:
        print("\n[SUCCESS] 达成率 >= 85%，目标完成！")
